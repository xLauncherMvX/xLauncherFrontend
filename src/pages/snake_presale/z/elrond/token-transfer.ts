...
export class TokenTransfer {
    ...
    protected constructor(options: ITokenTransferOptions) {
        ...
        if (!amount.isInteger() || amount.isNegative()) {
            throw new Error(`bad amountAsBigInteger: ${options.amountAsBigInteger}`);
        }
    ...
    static fungibleFromBigInteger(tokenIdentifier: string, amountAsBigInteger: BigNumber.Value, numDecimals = 0) {
        return new TokenTransfer({
            tokenIdentifier,
            nonce: 0,
            amountAsBigInteger,
